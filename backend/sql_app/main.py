...
def create_user(user: schemas.UserCreate, db: Session = Depends(get_db)):
    ...
    # Reviso si los password matchean
    if user.password != user.repeat_password:
        raise HTTPException(status_code=400, detail="Passwords must match!")
    # reviso si el password contiene el regex requerido
    if not re.match("^(?=.*?[A-Z])(?=.*?[a-z]).{8,}$", user.password):
        raise HTTPException(status_code=400, detail="Invalid Password, it must contain at least 8 characters, one uppercase letter and one lowercase letter")
...
@api.put("/tasks/{task_id}", response_model=schemas.Task)
...
@api.delete("/tasks/{task_id}", response_model=schemas.Task)
